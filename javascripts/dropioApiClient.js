...
function DropioApiClient(the_api_key,the_xd_path,opts) {
	...
	DropioApiClient.version = this.version;
    ...
    *************************************************************************************************/
    this.uploadFileForm = function(params,callback,options) {
	  ...
	  params.drop_name = drop_name;

	  var hiddenFields = [];
	  for( key in params ) {
		hiddenFields.push(DOMHelper.createElement("input",{"type":"hidden","name":key,"value":params[key]}))
	  }
    ...
    ********************************/
    this.cleanParams = function(params) {
      ...
      for( var i in params ) {
        if(typeof(params[i]) == "boolean" || (params[i] != null && params[i] != ""))	
...
DropioApiClient.getRedirectUrl = function(url,params) {
  ...
  redirectTo = url + "?version="+DropioApiClient.version+"&signature=" + sig + "&expires=" + exp;
  if( params.redirect_to )
	redirectTo += "&redirect_to=" + params.redirect_to;
